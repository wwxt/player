<template>
	...
	<el-row>
		<el-col :span="24">
			...
			<div class="po-top"  v-show="isShow"><i class="el-icon-close" @click="goDatail"></i></div>
	...
	export default{
		...
		created(){
			...
			this.attention= attent == undefined ? [] : JSON.parse(attent);
				var x=this.$route.params.are
				var arrHot=JSON.parse(localStorage.getItem("arrHot1"))[x];
					for(var i =0 ;i<arrHot.itemList.length;i++){
						if(this.$route.params.pid == arrHot.itemList[i].data.id  ){
							this.dataitemList=arrHot.itemList[i].data
							break;
						}
			}
			//点击进来根据ID获取的数据
